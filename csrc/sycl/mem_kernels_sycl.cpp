...
void multi_layer_kv_transfer(
    ...
    const GPUKVFormat gpu_kv_format, const int block_size,
    const int skip_prefix_n_tokens) {
  // NOTE: H2D staging (CPU → device bulk copy before scatter) is now
  // handled by the Python connector layer, which pre-allocates a reusable
  // device buffer to avoid per-call sycl::malloc_device / sycl::free
  // overhead.  The kernel assumes key_value is already on the target
  // device for H2D transfers.
...
void multi_layer_kv_transfer_unilateral(
  ...
  // NOTE: H2D staging is now handled by the Python connector layer.
